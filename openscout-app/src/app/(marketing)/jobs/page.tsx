  ...
  return (
    <div className="mx-auto max-w-4xl dark:bg-transparent">
      ...
      {!listings || listings.length === 0 ? (
        <div className="mt-12 rounded-[10px] border border-dashed border-gray-300 p-12 text-center dark:border-zinc-700 dark:bg-zinc-900/50">
        ...
        <div className="mt-8 space-y-4">
          {listings.map((job: { id: string; title: string; description: string | null; min_cv_score: number | null; companies: unknown }) => (
            <Link key={job.id} href={`/jobs/${job.id}`}>
              <div className="rounded-[10px] border border-[var(--border)] bg-white p-6 shadow-soft transition-shadow hover:shadow-card dark:border-white/[0.06] dark:bg-zinc-900 dark:hover:shadow-card">
