...
import { getInterviewGuideContent } from "@/lib/seo/content";
...
export async function generateMetadata({ params }: Props) {
  ...
  return buildSEOMetadata({
    title: `${title} Interview Guide | OpenScout`,
    description: `How to prepare for your ${title} interview: before, during, and after. Tips and resources. Practice with AI on OpenScout.`,
    path: `/interview-guide/${slug}`,
  });
}
...
export default async function InterviewGuidePage({ params }: Props) {
  ...
  const content = getInterviewGuideContent(jobTitle);

  return (
    <SEOPageLayout title={`${jobTitle} Interview Guide`}>
      <section>
